...
export const exportToDoc = (text: string, filename: string) => {
  const header = `
    ...
    <head><meta charset='utf-8'><title>Transcription Export</title></head><body>
    <div style='font-family: "Segoe UI", Tahoma, sans-serif; padding: 40px;'>
      ...
      <div>
        <h2 style='text-align: center; text-decoration: underline; color: #333; margin-bottom: 20px;'>TRANSCRIPTION OUTPUT</h2>
        <div style='white-space: pre-wrap; font-size: 12pt; line-height: 2.0; color: #000;'>${text}</div>
      </div>
      <div style='margin-top: 100px; text-align: right;'>
        <p style='color: #666; font-size: 10px;'>Generated via 2S Tec Judicial Node on ${new Date().toLocaleString()}</p>
